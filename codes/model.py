...
class SeedGenerator(nn.Module):   # 定义了如何生成种子点，十分重要！！！
...
class UpTransformer(nn.Module):
    ...
    def forward(self, pos, key, query, upfeat):
        ...
        value = self.mlp_v(torch.cat([key, query], 1)) # (B, dim, N)  与论文对应，先将k，q concat起来然后过MLP层
        identity = value
        ...
        key = grouping_operation(key, idx_knn)  # (B, dim, N, k)  使用pointnet2中的分组操作，有点聚类的意思
        qk_rel = query.reshape((b, -1, n, 1)) - key   # 对应文章中的upsample_transformer block中的qk经过relation操作
        ...
        # softmax function
        attention = self.scale(attention)  # 过softmax函数
...
class SeedFormer(nn.Module):
    ...
    def forward(self, partial_cloud):
        ...
        Args:
            partial_cloud: (B, N, 3)
        """
        # 输入点云在开始前要经过FPS下采样到2048个点
        # Encoder 和文章中对应通过encoder得到patches和两个feature，patch_feat是256个通道，feat是默认1024维
        feat, patch_xyz, patch_feat = self.forward_encoder(partial_cloud)

        # Decoder，包含了seed generator(重点)和上采样过程
